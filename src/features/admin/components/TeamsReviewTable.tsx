...
import {
  buildDuplicateRosterAlerts,
  buildTeamApprovalReviews,
  type TeamApprovalReview
} from "@/lib/domain/team-review";
...
export function TeamsReviewTable({ data }: { data: CompetitionData }) {
  ...
  const reviewByTeamId = useMemo(
    () => buildTeamApprovalReviews({ events: data.events, teams, players: data.players }),
...
const emptyReview: TeamApprovalReview = {
  event: null,
  players: [],
  playerCount: 0,
  issues: [],
