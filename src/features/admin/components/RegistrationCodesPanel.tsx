...
export function RegistrationCodesPanel() {
  ...
  async function loadCodes() {
    ...
    const payload = (await response.json().catch(() => null)) as CodesPayload | null;

    if (!response.ok || !payload?.ok) {
      toast.error("No se pudieron cargar los codigos.");
      return;
    }
  ...
  async function createCodes() {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { ok?: boolean; error?: string; codes?: CodeRow[] } | null;
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      <SectionHeader
        ...
        description="Codigos por campeonato, validos para cualquier categoria del campeonato."
      ...
      <Card className="p-5">
        <div className="grid gap-3 md:grid-cols-[1fr_140px_120px_auto] md:items-end">
          ...
          <Field label="Metodo">
            <select className={inputClass} value={method} onChange={(event) => setMethod(event.target.value as "yape" | "plin")}>
              <option value="yape">Yape</option>
              <option value="plin">Plin</option>
            </select>
          </Field>
          <Field label="Cantidad">
            <input className={inputClass} type="number" min={1} max={100} value={count} onChange={(event) => setCount(Number(event.target.value) || 1)} />
          </Field>
      ...
      <Card className="overflow-hidden">
        <div className="border-b border-ink/10 p-5">
          <SectionHeader
            ...
            action={
              <Button variant="secondary" onClick={() => void copyAvailableCodes()} disabled={availableCodes.length === 0}>
